...
fetch('https://jsonplaceholder.typicode.com/users').then(function (response) {
  ...
  document.getElementById('allUsers').innerHTML = usersArray.map(function (user) {
    return "<div>\n            <div id=\"user-card\">\n            <h3>".concat(user.username, "</h3>\n            <div class = \"user-email\">").concat(user.email, "</div>\n            <button id = \"btnMore\" onclick = \"getElementById('moreInfo').innerHTML='ID: ").concat(user.id, "<br> Address: ").concat(user.address.street, " <br> Phone: ").concat(user.phone, " <br> Website: ").concat(user.website, " <br> Company: ").concat(user.company.name, "'\">More...</button>\n            <div id = \"demo\"></div>\n            </div>\n        </div>");
  }).join('');
});
